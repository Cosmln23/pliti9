...
export async function triggerPaymentSuccessNotification(data: PaymentWebhookData): Promise<boolean> {
  try {
    ...
    console.log('🎯 DESTINATARI (LA CINE SĂ TRIMIȚI):')
    console.log('{{1.email}} = (DESTINATAR EMAIL)', payload.email)
    console.log('{{1.phoneNumber}} = (DESTINATAR WHATSAPP)', payload.phoneNumber)
    console.log('📝 DATE PENTRU PERSONALIZARE:')
    ...
    console.log('📧 MESAJE COMPLETE GATA DE TRIMITERE:')
    console.log('{{1.emailTemplate}} = (MESAJ COMPLET PENTRU EMAIL)', payload.emailTemplate ? 'GENERAT' : 'NULL')
    console.log('{{1.whatsappTemplate}} = (MESAJ COMPLET PENTRU WHATSAPP)', payload.whatsappTemplate ? 'GENERAT' : 'NULL')
